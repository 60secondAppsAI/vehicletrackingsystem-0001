					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateVehicle()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div id="app">
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Vehicles = true">Add</base-button>
								              <modal :show.sync="modal2Vehicles">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Vehicle Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="VehicleId" type="text" placeholder="Enter VehicleId" v-model="vehicleDetails.vehicleId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">License Plate</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="LicensePlate" type="text" placeholder="Enter LicensePlate" v-model="vehicleDetails.licensePlate"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Model</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Model" type="text" placeholder="Enter Model" v-model="vehicleDetails.model"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateVehicle() {
      ...
      const jsonData = JSON.stringify(this.vehicleDetails);
      const res = await VehicleService.update(jsonData);
...
//        this.getAllVehicles();
    ...
    async getVehicleDetails() {
      try {
			let response = await VehicleService.get(this.vehicleId);
